...
export interface Locale extends ILocale {
    ...
    "_mkgoIpRelated": {
        ...
        /**
         * 並び順は共有したIPアドレスの重みの合計です。重みは古い観測ほど小さく、半減期は{n}日です。出しているのはそれぞれの最終観測で、両者が同じ時間帯に接続していたことを示すものではありません。並び順はアカウント数を考慮していないので、同じ期間にそのIPアドレスを使ったアカウントが多いほど一致は弱い手掛かりである点は、各候補のアカウント数を見て判断してください。
         */
        "rankingBasis": ParameterizedString<"n">;
        /**
         * アカウント数が「{n}件以上」と出ている候補では実際の数がそれより大きく、候補どうしの比較には使えません。
         */
        "accountCountCapNote": ParameterizedString<"n">;
        /**
        ...
        "noneResolvable": string;
        /**
         * 集めた範囲の候補はすべて削除済みで表示できませんでした。ただし上の打ち切りにより全部は調べていないため、他に候補が残っている可能性があります。
         */
