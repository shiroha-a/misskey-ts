...
export interface Locale extends ILocale {
    ...
    "_mkgoIpRelated": {
        ...
        "disclaimer": string;
        /**
         * この期間に対象が使ったIPアドレスを起点に、同じ期間内にそのIPアドレスを使った他のローカルアカウントを探します。期間は対象側と候補側の両方に掛かります。
         */
        "periodMeaning": string;
        /**
         * 起点にしたIPアドレス
        ...
        /**
         * この期間にこのIPアドレスを使ったアカウント: {n}件
         */
        "ipAccountCount": ParameterizedString<"n">;
        /**
         * この期間にこのIPアドレスを使ったアカウント: {n}件以上
         */
        "ipAccountCountAtLeast": ParameterizedString<"n">;
        /**
         * 古い側の観測から{n}日
         */
        "elapsedDays": ParameterizedString<"n">;
        /**
         * 並び順は共有したIPアドレスの重みの合計です。重みは古い観測ほど小さく、半減期は{n}日です。出しているのはそれぞれの最終観測で、両者が同じ時間帯に接続していたことを示すものではありません。**並び順はアカウント数を考慮していません** — 同じ期間にそのIPアドレスを使ったアカウントが多いほど一致は弱い手掛かりなので、並び順だけで判断せず各候補のアカウント数も見てください。ただしアカウント数は上限で頭打ちになることがあり、「{m}件以上」と出ている場合は実際の数がそれより大きく、候補どうしの比較には使えません。
         */
        "rankingBasis": ParameterizedString<"n" | "m">;
        /**
        ...
        "noneResolvable": string;
        /**
         * 集めた範囲の候補はすべて削除済みで表示できませんでした。ただし候補が多く途中で打ち切っているため、集めていない候補が残っている可能性があります。
         */
        "noneResolvablePartial": string;
        /**
         * この利用者が直近{n}日にIPアドレスを記録された形跡がありません。起点が無いため、他のアカウントとの照合を行っていません。期間を広げるか、IPアドレスの記録が有効かを確認してください。
         */
        "noTargetRecords": ParameterizedString<"n">;
        /**
         * 調べた範囲では他のローカルアカウントは見つかりませんでした。上の打ち切りにより全部は調べていないので、関連が無いとは言えません。
         */
        "noneInSearchedRange": string;
        /**
         * このほかに{n}件の候補がありますが、該当するアカウントが削除済みのため表示していません。
         */
        "droppedNote": ParameterizedString<"n">;
        /**
         * ページを送っている間に新しい観測が入ると並びが変わり、候補が前のページへ移ることがあります。番号は表示順で、順位そのものではありません。
         */
        "pagingNote": string;
        /**
         * 表示順 {n}
         */
        "displayOrder": ParameterizedString<"n">;
        /**
         * 候補が多いため、一部のIPアドレスについて候補を途中までしか集めていません。表示している並び順とその根拠は不完全で、集めていない候補のほうが手掛かりとして強い可能性があります。そのIPアドレスを使ったアカウント数も「以上」の表示になります。この打ち切りはページを送っても解消しません。
         */
        "candidatesTruncated": string;
        /**
         * 対象が使ったIPアドレスが多いため、最近の{n}件だけを起点にしました。それより前のIPアドレスは一度も照合していません。期間を絞ると起点を絞り込めます。
         */
        "targetIpsTruncated": ParameterizedString<"n">;
        /**
         * (上限)
         */
        "capped": string;
